...
// Embedded conversation questions data to avoid module issues in production
const questionsData = [
  ...
  { category: "trust_boundaries", intensity: 1, questionText: "What is one area where you fully trust your partner?", therapyPrompt: "Name a boundary that protects connection, not distance." },
  { category: "trust_boundaries", intensity: 2, questionText: "How do you balance independence with togetherness?", therapyPrompt: "Name a boundary that protects connection, not distance." },
  { category: "trust_boundaries", intensity: 2, questionText: "What boundary would help your relationship thrive?", therapyPrompt: "Name a boundary that protects connection, not distance." },
  { category: "trust_boundaries", intensity: 1, questionText: "How do you honor your partner's privacy while staying connected?", therapyPrompt: "Name a boundary that protects connection, not distance." },
  { category: "trust_boundaries", intensity: 2, questionText: "What does emotional safety mean in your relationship?", therapyPrompt: "Name a boundary that protects connection, not distance." }
];
...
export async function seedConversationQuestions() {
  ...
  try {
    ...
    if (existing.length > 0) {
      console.log(`Questions already seeded (${existing.length} found), skipping...`);
      return { status: 'already_seeded', count: existing.length };
    }

    // Insert all 140 questions
    for (const question of questionsData) {
      await storage.createConversationQuestion({
        category: question.category,
        intensity: question.intensity,
        questionText: question.questionText,
        therapyPrompt: question.therapyPrompt,
      });
    }
    
    console.log(`Successfully seeded ${questionsData.length} conversation questions!`);
    return { status: 'seeded', count: questionsData.length };
  } catch (error) {
    console.error("Error seeding conversation questions:", error);
    throw error;
  }
}
