...
- **Favicon & SEO Optimization** (November 2025): Enhanced favicon configuration with multiple size declarations for better browser support across all devices. Updated SEO component default Open Graph image from non-existent og-image.jpg to actual twangle-logo.png. Fixed canonical URLs to use twangle.com domain consistently. Added `.crisp-image` utility class to index.css with hardware-accelerated rendering for sharp logo display across all browsers (Chrome, Firefox, Safari, Edge). Landing page logo optimized with `image-rendering: crisp-edges` and `-webkit-optimize-contrast` for pixel-perfect clarity on all screen densities. Favicon now appears properly in browser address bar tabs, bookmarks, and search engine results.
- **UI Refinements** (November 2025): Repositioned FeedbackButton from inside AppHeader to fixed position below header (top-right corner) to prevent overlap with header buttons on small screens. Removed duplicate Daily Conversations FeatureCard from landing page features grid since there's already a dedicated featured card section for it. Updated logo to transparent PNG (twangle-logo.png) at h-15 (60px) recommended size in header.
