...
- **UI Refinements** (November 2025): Repositioned FeedbackButton from inside AppHeader to fixed position below header (top-right corner) to prevent overlap with header buttons on small screens. Removed duplicate Daily Conversations FeatureCard from landing page features grid since there's already a dedicated featured card section for it. Logo sizes optimized: AppHeader (48px), Landing page (80-96px), Login/Signup (64-80px).
