# Social Media Signup Links for Twangle

Use these links to promote Twangle on social media. Each link includes tracking to show which platform drove the signup.

## 📘 Facebook

**Bio Link / Posts:**
```
https://twangle.com/signup?utm_source=facebook&utm_medium=social&utm_campaign=signup
```

**Facebook Ads:**
```
https://twangle.com/signup?utm_source=facebook&utm_medium=paid&utm_campaign=ad
```

**Suggested Post Copy:**
```
💕 Stop fighting about the same things. Start understanding each other.

Twangle helps couples discover their attachment patterns and build stronger relationships with AI-powered coaching.

✨ 7-day free trial
🔒 Private & secure
💳 No credit card required

Start your free trial: https://twangle.com/signup?utm_source=facebook
```

---

## 🐦 Twitter/X

**Bio Link:**
```
https://twangle.com/signup?utm_source=twitter&utm_medium=social&utm_campaign=signup
```

**Tweet Links:**
```
https://twangle.com/signup?utm_source=twitter&utm_medium=tweet&utm_campaign=signup
```

**Suggested Tweet Copy:**
```
Stuck in the same arguments? 💔

Twangle uses AI + attachment theory to help couples:
• Understand relationship patterns
• Improve communication  
• Build lasting connection

Start your 7-day free trial (no credit card)
https://twangle.com/signup?utm_source=twitter
```

---

## 📸 Instagram

**Bio Link:**
```
https://twangle.com/signup?utm_source=instagram&utm_medium=social&utm_campaign=signup
```

**Stories / Posts (use Link Sticker):**
```
https://twangle.com/signup?utm_source=instagram&utm_medium=story&utm_campaign=signup
```

**Instagram Link in Bio Services** (Linktree, etc.):
```
Title: Start Your Free Trial
URL: https://twangle.com/signup?utm_source=instagram&utm_medium=bio&utm_campaign=linktree
```

**Suggested Caption:**
```
💕 Transform your relationship in 7 days

Twangle combines AI coaching with proven therapy methods (Gottman Method, Attachment Theory, EFT) to help couples:

✨ Discover your attachment style
🗣️ Improve communication
❤️ Build deeper connection
📊 Track relationship health

7-day free trial • No credit card • Private & secure

Link in bio 👆 or visit twangle.com/signup
```

---

## 🎵 TikTok

**Bio Link:**
```
https://twangle.com/signup?utm_source=tiktok&utm_medium=social&utm_campaign=signup
```

**Video Links (in comments or video overlay):**
```
https://twangle.com/signup?utm_source=tiktok&utm_medium=video&utm_campaign=signup
```

**Suggested Bio:**
```
💕 AI relationship coach for couples
7-day free trial → twangle.com/signup
```

**Video Ideas:**
- "Discover your attachment style in 5 minutes"
- "Why you fight about the same things (and how to stop)"
- "What your relationship patterns reveal about you"
- "7 days to a healthier relationship"

---

## 📌 Pinterest

**Pin Links:**
```
https://twangle.com/signup?utm_source=pinterest&utm_medium=social&utm_campaign=pin
```

**Board Description:**
```
Relationship coaching resources for couples | Discover your attachment style | AI-powered guidance
Visit: https://twangle.com/signup?utm_source=pinterest
```

---

## 💼 LinkedIn

**Profile Link:**
```
https://twangle.com/signup?utm_source=linkedin&utm_medium=social&utm_campaign=signup
```

**Post Links:**
```
https://twangle.com/signup?utm_source=linkedin&utm_medium=post&utm_campaign=signup
```

---

## 📱 Direct Signup Links by Platform

Quick reference for sharing:

| Platform | Direct Link |
|----------|-------------|
| Facebook | `https://twangle.com/signup?utm_source=facebook` |
| Twitter/X | `https://twangle.com/signup?utm_source=twitter` |
| Instagram | `https://twangle.com/signup?utm_source=instagram` |
| TikTok | `https://twangle.com/signup?utm_source=tiktok` |
| Pinterest | `https://twangle.com/signup?utm_source=pinterest` |
| LinkedIn | `https://twangle.com/signup?utm_source=linkedin` |

---

## 🎯 How It Works

When users click these links:
1. They land on a dedicated signup page optimized for mobile
2. A small emoji indicator shows which platform they came from (builds trust)
3. You can track which platforms drive the most signups in your analytics
4. Users get a streamlined signup flow with social login options

---

## 🔥 Pro Tips

### For Best Results:
- ✅ Use the shortened links in character-limited platforms (Twitter)
- ✅ Test links before posting to ensure they work
- ✅ Update UTM parameters to track specific campaigns (e.g., `utm_campaign=valentines`)
- ✅ Pin the signup link in your bio on all platforms
- ✅ Include a clear call-to-action in every post

### What the Signup Page Shows:
- 📱 Mobile-optimized design
- 🎨 Platform-specific emoji indicator (e.g., 📘 for Facebook)
- ⚡ One-click Google/Facebook login
- 📝 Simple email signup form
- ✨ "7 days free, no credit card" messaging
- 🔒 Trust indicators (secure, private, cancel anytime)

---

## 📊 Tracking Your Results

After users sign up via these links, you can track:
- Which platform drives the most signups
- Which campaigns perform best
- Conversion rates by source
- User behavior after signup

Check your analytics dashboard to see real-time signup data by source!
